...
// db connectio
try {
    $connection = Database::getInstance('localhost', 'hospitaldb', 'root', '');

    // doctor migration table
    $doctor = new Migration(clone $connection, 'doctors');
    $doctor->addColumn('DoctorID', 'int', 11, [Constraints::PrimaryKey]);
    ...
    $patient = new Migration(clone $connection, 'patients');
    $patient->addColumn('PatientID', 'int', 11, [Constraints::PrimaryKey]);
    ...
    $appointment = new Migration(clone $connection, 'appointments');
    $appointment->addColumn('AppointmentID', 'int', 11, [Constraints::PrimaryKey]);
    $appointment->addColumn('PatientID', 'int', 11, [Constraints::ForeignKey], ['patients' => 'PatientID']);
    $appointment->addColumn('DoctorID', 'int', 11, [Constraints::ForeignKey], ['patients' => 'PatientID']);
